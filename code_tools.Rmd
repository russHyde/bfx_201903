...
- [py] black / pylint; [R] styler / lintr
...
So there are automated tools for

- defining package structure

- cleaning up your code & identifying stylistic problems

- running your (manually curated) tests

- finding additional tests

Sphinx helps automate the generation of documentation
...
```
# - Setup a package structure
# - Add some source code, with docstrings associated with the
# modules / classes / functions

# Initialise the docs file structure
cd my_pkg
mkdir docs && cd docs
...
# Modify source/conf.py, swear a lot, question your sanity and try again
```
...
## Use a more full-featured python-package cookie-cutter

Set up `rosa_with_docs` project with the following:

```
cookiecutter git@github.com:audreyr/cookiecutter-pypackage.git
```

This cookiecutter comes with a Makefile covering linting, testing, installation
and doc creation.
...
project_name [Python Boilerplate]: rosa_with_docs
project_slug [rosa_with_docs]: 
...
```{bash, echo = TRUE}
(cd rosa_with_docs && make test) # using pytest
```
...
# Not ran
(cd rosa_with_docs && make lint) # using flake8
```
